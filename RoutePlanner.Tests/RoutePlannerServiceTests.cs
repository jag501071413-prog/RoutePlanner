...
namespace RoutePlanner.Tests
{
    ...
    public class RoutePlannerServiceTests
    {
        ...
        public void Setup()
        {
            _planner = new RoutePlannerService(new HaversineService());
        }
        ...
        public void BuildRoute_SimpleFeasibleSchedule_ReturnsAllStops()
        {
            ...
            // start + two visits + end
            Assert.That(result.Stops.Count, Is.EqualTo(4));
        ...
        {
            // Appointment window that's impossible because it's already past end before travel
            var appointments = new List<Appointment>
            {
                new Appointment
                {
